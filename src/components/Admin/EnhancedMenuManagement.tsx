...
/**
 * Enhanced Menu Management Component
 * Supports categories, items, variations (sizes), and add-ons (modifiers)
...
export default function EnhancedMenuManagement() {
  ...
  const [variationForm, setVariationForm] = useState<Partial<ItemVariation>>({});
  ...
  const handleSaveItem = async () => {
    try {
      ...
      if (editingItem) {
        ...
        await createMenuItem(itemForm as { name: string; description?: string | null; price: number; category_id?: string | null; is_available?: boolean | null; image_url?: string | null });
  ...
  const handleSaveVariation = async () => {
    try {
      if (!variationForm.menu_item_id) {
        toast.error('Please select a menu item');
        return;
      }

      if (editingVariation) {
        await updateItemVariation(editingVariation.id, variationForm);
        toast.success('Variation updated!');
      } else {
        await createItemVariation(variationForm as {
          menu_item_id: string;
          name: string;
          price_adjustment: number;
        });
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-white">Enhanced Menu Management</h2>
            <p className="text-gray-400">Manage categories, items, sizes, and add-ons</p>
          </div>
        </div>
        <Button
          onClick={fetchData}
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                ...
                {menuItems.map((item) => {
                  ...
                  return (
                    <TableRow key={item.id} className="border-gray-700">
                      ...
                      <TableCell>
                        <Badge variant="secondary">{itemVars.length} sizes</Badge>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            ...
                            onClick={() => {
                              setSelectedItem(item);
                              setVariationForm({ menu_item_id: item.id });
        ...
        <TabsContent value="addons" className="space-y-4">
          <div className="flex justify-between items-center">
            <h3 className="text-lg font-semibold text-white">Add-ons & Modifiers</h3>
            <Button
              onClick={() => {
                setEditingAddon(null);
                setAddonForm({});
          ...
          <Card className="bg-gray-800 border-gray-700">
            <Table>
              ...
              <TableBody>
                {addons.map((addon) => {
                  const links = addonLinks.filter(l => l.addon_id === addon.id);
                  ...
                  return (
                    <TableRow key={addon.id} className="border-gray-700">
                      ...
                      <TableCell>
                        <Badge variant="secondary">{links.length} items/categories</Badge>
                      </TableCell>
                      <TableCell>
        ...
        <TabsContent value="variations" className="space-y-4">
          <div className="flex justify-between items-center">
            <h3 className="text-lg font-semibold text-white">Item Variations (Sizes)</h3>
            <Button
              onClick={() => {
                setEditingVariation(null);
                setVariationForm({});
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-gray-700">
                  ...
                  <TableHead className="text-gray-300">Variation Name</TableHead>
                  <TableHead className="text-gray-300">Price Adjustment</TableHead>
              ...
              <TableBody>
                {variations.map((variation) => {
                  const item = menuItems.find(i => i.id === variation.menu_item_id);

                  return (
                    <TableRow key={variation.id} className="border-gray-700">
                      <TableCell className="text-white">{item?.name || 'Unknown'}</TableCell>
                      <TableCell className="text-gray-300 font-medium">{variation.name}</TableCell>
                      <TableCell className={variation.price_adjustment >= 0 ? 'text-green-400' : 'text-red-400'}>
                        {variation.price_adjustment >= 0 ? '+' : ''}R{variation.price_adjustment}
                      </TableCell>
                      <TableCell>
                        {variation.is_default && <Badge variant="default">Default</Badge>}
                      </TableCell>
                      <TableCell>
        ...
        <DialogContent className="bg-gray-900 border-gray-700 text-white max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              ...
              {selectedItem && <span className="text-gray-400 ml-2">for {selectedItem.name}</span>}
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-4">
            {!selectedItem && (
              <div>
                ...
                <Select
                  value={variationForm.menu_item_id || ''}
                  onValueChange={(value) => setVariationForm({ ...variationForm, menu_item_id: value })}
                >
                  <SelectTrigger className="bg-gray-800 border-gray-600 text-white">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label>Variation Name * (e.g., "Small", "Medium", "Large")</Label>
                <Input
              ...
              <div>
                ...
                <Input
                  type="number"
                  step="0.01"
                  value={variationForm.price_adjustment || 0}
                  onChange={(e) => setVariationForm({ ...variationForm, price_adjustment: parseFloat(e.target.value) })}
                  className="bg-gray-800 border-gray-600 text-white"
                  placeholder="e.g., -10 or +15"
                />
                <p className="text-xs text-gray-400 mt-1">
                  Positive for more expensive, negative for cheaper
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  type="number"
                  value={variationForm.display_order || 0}
                  onChange={(e) => setVariationForm({ ...variationForm, display_order: parseInt(e.target.value) })}
                  className="bg-gray-800 border-gray-600 text-white"
                />
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center space-x-2 pt-2">
                  <input
                    type="checkbox"
                    checked={variationForm.is_default || false}
                    onChange={(e) => setVariationForm({ ...variationForm, is_default: e.target.checked })}
                    className="rounded"
                  />
                ...
                <div className="flex items-center space-x-2">
                  <input
                    type="checkbox"
                    checked={variationForm.is_available !== false}
                    onChange={(e) => setVariationForm({ ...variationForm, is_available: e.target.checked })}
                    className="rounded"
                  />
                  <span className="text-sm">Available</span>
                </div>
            ...
            {/* Show existing variations for this item */}
            {selectedItem && (
              <div className="border-t border-gray-700 pt-4">
                <Label className="text-gray-400 mb-2 block">Existing Variations for {selectedItem.name}:</Label>
                <div className="space-y-1">
                  {getItemVariations(selectedItem.id).map((v) => (
                    <div key={v.id} className="flex items-center justify-between bg-gray-800 px-3 py-2 rounded">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button onClick={handleSaveVariation} className="bg-neonCyan text-black">
